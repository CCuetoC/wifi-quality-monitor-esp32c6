...
// Helper para CSS (V3.0 - Perfección Estética Industrial)
String getCommonCSS() {
    String c = "<style>body{background:#080808;color:#ddd;font-family:'Segoe UI',sans-serif;margin:0;padding-bottom:50px;}";
    c += ".top-bar{background:#111;padding:15px 25px;border-bottom:3px solid #00ffcc;display:flex;justify-content:space-between;align-items:center;max-width:1100px;margin:0 auto 20px;box-sizing:border-box;}";
    c += ".title{color:#00ffcc;font-weight:bold;font-size:1.1em;letter-spacing:1px;} .clock{color:#888;font-family:monospace;font-size:0.95em;}";
    c += ".nav{display:flex;gap:15px;} .nav a{color:#00ffcc;font-weight:bold;font-size:0.8em;padding:8px 15px;border:1px solid #333;border-radius:6px;text-decoration:none;background:#1a1a1a;transition:0.3s;} .nav a:hover{background:#00ffcc;color:#000;}";
    c += ".grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;padding:0;max-width:1100px;margin:0 auto 25px;}";
    c += ".card{background:#111;padding:15px;border-radius:10px;border:1px solid #222;box-shadow:0 4px 10px rgba(0,0,0,0.5);} .card div:first-child{font-size:0.7em;color:#666;margin-bottom:6px;text-transform:uppercase;} .val{font-size:1.1em;font-weight:bold;color:#fff;}";
    c += "h2{font-size:1em;color:#00ffcc;margin:35px auto 12px;text-align:left;max-width:1100px;padding:0 10px;text-transform:uppercase;letter-spacing:1px;}";
    c += ".chart-box{background:#111;padding:20px 15px;border-radius:12px;border:1px solid #222;max-width:1100px;margin:10px auto;display:flex;align-items:center;}";
    c += ".canvas{flex-grow:1;height:200px;background:#050505;border:1px solid #333;margin:0 15px;position:relative;padding-top:10px;}";
    c += ".axis-l{width:50px;height:200px;display:flex;flex-direction:column;justify-content:space-between;font-size:0.75em;color:#555;}";
    c += ".axis-r{width:100px;height:200px;display:flex;flex-direction:column;justify-content:space-between;font-size:0.65em;color:#0fc;text-align:left;padding-left:10px;}";
    c += ".axis-r div{height:25%;display:flex;align-items:center;}";
    c += ".log-container{max-width:1100px;margin:20px auto;border:1px solid #333;border-radius:10px;overflow:hidden;background:#111;}";
    c += ".log-grid{display:grid;grid-template-columns:115px 115px 180px 1fr;text-align:left;font-family:monospace;font-size:1.05em;}";
    c += ".log-h{background:#00ffcc;color:#000;font-weight:bold;padding:15px;position:sticky;top:0;z-index:10;}";
    c += ".log-scroll{height:550px;overflow-y:auto;} .log-row{padding:14px 12px;border-bottom:1px solid #222;}";
    c += ".tag{padding:3px 6px;border-radius:4px;font-size:0.75em;font-weight:bold;} .CRITICAL{background:#f44;} .STATE_CHANGE{background:#0fc;color:#000;} .HEARTBEAT{color:#444;}";
    c += "</style>";
    return c;
}
...
void NetworkService::update() {
    ...
    // CALIBRACIÓN V3.0: Huso horario de Lima (UTC-5) nativo
    ...
    if (hasTime && !_bootReasonLogged) {
        estimateLastPowerOff(); checkStartupReason();
        logEvent("SYS_STATUS", "Link Active - System Ready");
        _bootReasonLogged = true;
    }

    if (_fsReady && hasTime && (millis() - _lastHeartbeatSent > 30000)) {
        char buf[64]; sprintf(buf, "ALIVE | RSSI: %d | IP: %s", WiFi.RSSI(), WiFi.localIP().toString().c_str());
        logEvent("HEARTBEAT", buf); _lastHeartbeatSent = millis();
    }
...
void NetworkService::estimateLastPowerOff() {
    if (!_fsReady) return;
    File f = LittleFS.open("/log.txt", FILE_READ); if (!f) return;
    size_t sz = f.size(); if (sz > 2048) f.seek(sz - 2048);
    String data = f.readString(); f.close();
    int lastH = data.lastIndexOf("HEARTBEAT");
    if (lastH > 20) {
        int lineStart = data.lastIndexOf('\n', lastH); if (lineStart == -1) lineStart = 0; else lineStart++;
        String line = data.substring(lineStart, data.indexOf('\n', lineStart));
        int yy, mm, dd, h, m, s;
        if (sscanf(line.c_str(), "%d-%d-%d|%d:%d:%d", &yy, &mm, &dd, &h, &m, &s) == 6) {
            struct tm tm = {0}; tm.tm_year = yy - 1900; tm.tm_mon = mm-1; tm.tm_mday = dd; tm.tm_hour = h; tm.tm_min = m; tm.tm_sec = s;
            time_t offTime = mktime(&tm) + 30; logEventWithTime(offTime, "POWER_OFF_EST", "Power loss detected");
        }
    }
}

void NetworkService::logEvent(const char* type, const char* data) { time_t n; time(&n); logEventWithTime(n, type, data); }
...
void NetworkService::_setupWebServer() {
    ...
    _server->on("/status", [this]() {
        ...
        String j = "{\"u\":\"" + getUptimeString() + "\",\"r\":" + String(d.rssi) + ",\"pg\":" + String(d.pingGW) + ",\"pn\":" + String(d.pingInternet) + ",\"qs\":" + String(_lastScore) + ",\"re\":" + String(_reconnectCount) + ",\"h\":" + String(fh/1024) + "}";
...
void NetworkService::_handleRoot() {
    ...
    h += "const m={'u':d.u,'r':d.r+' dBm','pg':d.pg+' ms','pi':d.pn+' ms','qs':d.qs+'%','re':d.re,'h':d.h+' KB'};";
    h += "for(let k in m){let e=document.getElementById(k);if(e)e.innerText=m[k];}}); }";
    h += "function ck(){let n=new Date();document.getElementById('clk').innerText=n.toISOString().split('T')[0]+' '+n.toTimeString().split(' ')[0];} setInterval(up,3000); setInterval(ck,1000);</script></head><body>";
    ...
    h += "<div class='card'><div>GATEWAY</div><div class='val' id='pg'>--</div></div><div class='card'><div>INTERNET</div><div class='val' id='pi'>--</div></div><div class='card'><div>RAM (FREE)</div><div class='val' id='h'>--</div></div><div class='card'><div>RECONS</div><div class='val' id='re'>--</div></div></div>";
    ...
    int hi[50], id; if(loadTrend(hi, 50, &id)) { h += "<polyline points='"; for(int i=0; i<50; i++) h += String(i*12) + "," + String(110-(hi[(id+i)%50])) + " "; h += "' fill='none' stroke='#0fc' stroke-width='1.5'/></svg></div>"; h += "<div class='axis-r'><div>EXCELLENT</div><div>GOOD</div><div>DEGRADED</div><div>CRITICAL</div></div></div>"; }
    h += "<h2>SYSTEM RAM (KB)</h2><div class='chart-box'><div class='axis-l'><div>320</div><div>240</div><div>160</div><div>80</div><div>0</div></div><div class='canvas'><svg width='100%' height='100%' viewBox='0 0 600 110' preserveAspectRatio='none'>";
    for(int y=10; y<=110; y+=25) h += "<line x1='0' y1='"+String(y)+"' x2='600' y2='"+String(y)+"' stroke='#222'/>";
    extern bool loadRam(int*, int*); int rh[50], rid; if(loadRam(rh, &rid)) { h += "<polyline points='"; for(int i=0; i<50; i++) h += String(i*12) + "," + String(110-(rh[(rid+i)%50])) + " "; h += "' fill='none' stroke='#48f' stroke-width='1.5'/></svg></div>"; h += "<div class='axis-r'><div>STABLE</div><div>NORMAL</div><div>SYSTEM</div><div>LOW</div></div></div>"; }
...
void NetworkService::_handleLogs() {
    ...
    int lineStart = data.indexOf('\n'); if (lineStart != -1) lineStart++; else lineStart = 0; String buf[80]; int count = 0;
...
void NetworkService::_performPing() {
    ...
    if (t == 0) { _lastPingGW = Ping.ping(g) ? Ping.averageTime() : -1; }
    else {
        bool gOk = Ping.ping("8.8.8.8"); int gTime = gOk ? Ping.averageTime() : -1;
        bool cOk = Ping.ping("1.1.1.1"); int cTime = cOk ? Ping.averageTime() : -1;
