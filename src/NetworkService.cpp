...
void NetworkService::_handleRoot(FileLogger& logger) {
    ...
    int hi[50], id; if(logger.loadTrend(hi, 50, &id)) { h += "<polyline points='"; for(int i=0; i<50; i++) h += String(i*12) + "," + String(110-(hi[(id+i)%50])) + " "; h += "' fill='none' stroke='#0fc' stroke-width='1.5'/></svg></div>"; h += "<div class='axis-r'><div>EXCELLENT</div><div>GOOD</div><div>DEGRADED</div><div>CRITICAL</div></div></div>"; }
    ...
    for(int y=10; y<=110; y+=25) h += "<line x1='0' y1='"+String(y)+"' x2='600' y2='"+String(y)+"' stroke='#222'/>";
    int rh[50], rid; if(logger.loadRam(rh, &rid)) { h += "<polyline points='"; for(int i=0; i<50; i++) h += String(i*12) + "," + String(110-((rh[(rid+i)%50]*100)/512)) + " "; h += "' fill='none' stroke='#48f' stroke-width='1.5'/></svg></div>"; h += "<div class='axis-r'><div>512 KB</div><div>384 KB</div><div>256 KB</div><div>128 KB</div><div>0 KB</div></div></div>"; }
...
void NetworkService::begin(const char* ssid, const char* pass) { _prefs.begin("net_stats", true); String s = _prefs.getString("w_ssid", ssid), p = _prefs.getString("w_pass", pass); _prefs.end(); WiFi.mode(WIFI_STA); WiFi.begin(s.c_str(), p.c_str()); _startTime = millis(); }
