...
// Helper para CSS (V2.4 - Perfección Industrial)
String getCommonCSS() {
    ...
    c += ".grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;padding:0 15px;max-width:1000px;margin:auto;}";
    ...
    c += ".val{font-size:1.15em;font-weight:bold;color:#fff;} h2{font-size:0.95em;color:#00ffcc;margin:25px auto 10px;text-align:left;max-width:1000px;padding-left:15px;}";
    c += ".chart-box{background:#111;padding:15px 15px 5px;border-radius:10px;border:1px solid #222;max-width:1000px;margin:10px auto;display:flex;align-items:center;position:relative;}";
    c += ".axis-l{width:40px;height:180px;display:flex;flex-direction:column;justify-content:space-between;font-size:0.7em;color:#666;padding-bottom:15px;}";
    c += ".axis-r{width:80px;height:180px;display:flex;flex-direction:column;justify-content:space-between;font-size:0.65em;color:#0fc;text-align:left;padding-left:10px;padding-bottom:15px;}";
    c += ".canvas{flex-grow:1;height:180px;background:#000;border:1px solid #333;position:relative;margin-bottom:15px;}";
    c += ".log-header{max-width:1000px;margin:15px auto 0;background:#00ffcc;color:#000;font-weight:bold;display:grid;grid-template-columns:100px 100px 120px 1fr;padding:10px;border-radius:8px 8px 0 0;font-size:0.85em;}";
    c += ".log-scroll{max-width:1000px;margin:0 auto;height:450px;overflow-y:auto;background:#111;border:1px solid #333;border-radius:0 0 8px 8px;}";
    c += ".log-row{display:grid;grid-template-columns:100px 100px 120px 1fr;padding:10px;border-bottom:1px solid #222;text-align:left;font-family:monospace;font-size:0.8em;}";
    c += ".tag{padding:2px 4px;border-radius:3px;font-size:0.7em;font-weight:bold;} .CRITICAL{background:#f44;} .STATE_CHANGE{background:#0fc;color:#000;} .btn{background:#0fc;color:#111;padding:12px;border:none;border-radius:5px;font-weight:bold;cursor:pointer;margin-top:10px;}";
    c += "</style>";
    return c;
}

String getNav() { return "<div class='nav'><a href='/'>DASHBOARD</a><a href='/logs'>AUDIT LOGS</a><a href='/config'>SETTINGS</a></div>"; }
...
void NetworkService::update() {
    ...
    if (_bootPhase == 0 && u > 10000) {
        ...
        _bootPhase = 1; logEvent("SYS_STATUS", "FS & NVS Ready");
    }
    if (_bootPhase == 1 && u > 15000) { if (!_server) _server = new WebServer(80); if (!_dnsServer) _dnsServer = new DNSServer(); _setupWebServer(); _bootPhase = 2; }
    if (_bootPhase == 2 && u > 20000) { configTime(_gmtOffset * 3600, 0, "pool.ntp.org", "time.google.com"); _bootPhase = 3; }
    ...
    if (c) {
        ...
        if (_bootPhase >= 1 && (millis() - _lastSaveTime > _saveInterval)) {
            _historicalUptime += (millis() - _lastSaveTime); _lastSaveTime = millis();
        ...
        if (millis() - _lastReconnectAttempt > _reconnectInterval) {
            _lastReconnectAttempt = millis(); static int f = 0; if (f < 2) WiFi.reconnect(); else WiFi.begin(); f++; _reconnectCount++;
...
void NetworkService::logEvent(const char* type, const char* data) {
    char d[16]="BOOT", t[16]="BOOT"; time_t n; time(&n);
    if (n > 1000000) { struct tm ti; localtime_r(&n, &ti); strftime(d, 16, "%Y-%m-%d", &ti); strftime(t, 16, "%H:%M:%S", &ti); }
    char msg[128]; sprintf(msg, "%s|%s|%s|%s", d, t, type, data);
    if (_fsReady) { _rotateLogs(); File f = LittleFS.open("/log.txt", FILE_APPEND); if (f) { f.println(msg); f.close(); } }
}
...
void NetworkService::_setupWebServer() {
    _server->onNotFound([this]() { if (_isConfigMode) { _server->sendHeader("Location", "/config", true); _server->send(302, "text/plain", ""); } else _server->send(404, "text/plain", "Not Found"); });
    ...
    _server->on("/status", [this]() {
        NetworkData d = getData(); uint32_t fh = ESP.getFreeHeap(); int hp = (fh * 100) / ESP.getHeapSize();
    ...
    _server->on("/save", HTTP_POST, [this]() {
        ...
        if (s.length() > 0) { _prefs.begin("net_stats", false); _prefs.putString("w_ssid", s); _prefs.putString("w_pass", p); _prefs.putInt("gmt", g.toInt()); _prefs.end(); _server->send(200, "text/html", "<b>Applied</b>. Rebooting..."); delay(1000); ESP.restart(); }
    });
    _server->begin();
}
...
void NetworkService::_handleRoot() {
    ...
    h += "<script>function up(){fetch('/status').then(r=>r.json()).then(d=>{try{";
    h += "const m={'u':d.u,'r':d.r+' dBm','pg':d.pg+' ms','pi':d.pn+' ms','qs':d.qs+'%','re':d.re,'h':(d.h/1024).toFixed(1)+' KB'};";
    h += "for(let k in m){let e=document.getElementById(k);if(e)e.innerText=m[k];}}catch(e){}}); } setInterval(up,2500);</script></head><body>";
    h += getNav() + "<h2>METRICS</h2><div class='grid'>";
    h += "<div class='card'><div>APP UPTIME</div><div class='val' id='u'>--</div></div><div class='card'><div>RSSI SIGNAL</div><div class='val' id='r'>--</div></div><div class='card'><div>QUALITY SCORE</div><div class='val' id='qs'>--</div></div>";
    h += "<div class='card'><div>PING GATEWAY</div><div class='val' id='pg'>--</div></div><div class='card'><div>PING INTERNET</div><div class='val' id='pi'>--</div></div><div class='card'><div>FREE RAM</div><div class='val' id='h'>--</div></div><div class='card'><div>RECONNECTS</div><div class='val' id='re'>--</div></div></div>";
    
    // SVG Gráfica de Calidad Profesional
    h += "<h2>WIFI QUALITY TREND</h2><div class='chart-box'><div class='axis-l'><div>100%</div><div>50%</div><div>0%</div></div>";
    h += "<div class='canvas'><svg width='100%' height='100%' viewBox='0 0 600 100' preserveAspectRatio='none'>";
    for(int y=25; y<=75; y+=25) h += "<line x1='0' y1='"+String(y)+"' x2='600' y2='"+String(y)+"' stroke='#222'/>";
    int hi[50], id; if(loadTrend(hi, 50, &id)) {
        h += "<polyline points='"; for(int i=0; i<50; i++) h += String(i*12) + "," + String(100-(hi[(id+i)%50])) + " "; h += "' fill='none' stroke='#0fc' stroke-width='3'/></svg></div>";
        h += "<div class='axis-r'><div>EXCELLENT</div><div>GOOD</div><div>CRITICAL</div></div></div>";
    }

    // SVG Gráfica de RAM Profesional
    h += "<h2>SYSTEM RAM TREND</h2><div class='chart-box'><div class='axis-l'><div>100%</div><div>50%</div><div>0%</div></div>";
    h += "<div class='canvas'><svg width='100%' height='100%' viewBox='0 0 600 100' preserveAspectRatio='none'>";
    for(int y=25; y<=75; y+=25) h += "<line x1='0' y1='"+String(y)+"' x2='600' y2='"+String(y)+"' stroke='#222'/>";
    int rh[50], rid; if(loadRam(rh, &rid)) {
        h += "<polyline points='"; for(int i=0; i<50; i++) h += String(i*12) + "," + String(100-(rh[(rid+i)%50])) + " "; h += "' fill='none' stroke='#48f' stroke-width='2'/></svg></div>";
        h += "<div class='axis-r'><div>HIGH</div><div>MID</div><div>LOW</div></div></div>";
    }
...
void NetworkService::_handleLogs() {
    ...
    _server->sendContent("<h2>System Audit Logs</h2><div class='log-header'><div>DATE</div><div>TIME</div><div>EVENT</div><div>DATA</div></div><div class='log-scroll'>");
    
    File f = LittleFS.open("/log.txt", FILE_READ);
    if (f.size() > 4000) f.seek(f.size() - 4000); // Cola inteligente del archivo
    String data = f.readString(); f.close();
    
    // Invertimos el orden para mostrar lo más nuevo arriba
    int lineStart = 0, lineEnd = 0, count = 0;
    String buf[60];
    while((lineEnd = data.indexOf('\n', lineStart)) > 0 && count < 60) {
        buf[count++] = data.substring(lineStart, lineEnd); lineStart = lineEnd + 1;
    }
...
void NetworkService::_handleConfig() {
    ...
    h += "d.forEach(n=>{h+='<div style=\"padding:8px;border-bottom:1px solid #222;cursor:pointer;\" onclick=\"sel(\\''+n.s+'\\')\">'+n.s+' ('+n.r+' dBm)</div>';});";
    h += "h+='</div>';document.getElementById('sl').innerHTML=h;b.innerText='Rescan';});} function sel(s){document.getElementById('ssid').value=s;}";
    h += "function tp(){const x=document.getElementById('pass');x.type=(x.type==='password')?'text':'password';}</script></head><body>";
    h += getNav() + "<h2>System Settings</h2><form action='/save' method='POST' style='display:inline-block;text-align:left;max-width:300px;'>";
    h += "SSID:<br><input type='text' id='ssid' name='ssid' style='width:200px;'><button type='button' id='sb' onclick='scan()' style='margin-left:5px;cursor:pointer;padding:8px;'>Scan</button>";
    h += "<div id='sl'></div><br>Pass:<br><input type='password' id='pass' name='pass'><br><input type='checkbox' onclick='tp()'> Show<br><br>";
    h += "GMT Offset:<br><input type='number' name='gmt' value='"+String(_gmtOffset)+"'><br><br>Admin Token:<br><input type='password' name='token' style='border:1px solid #0fc;'><br><br>";
